...
def init_populasi(popsize, gen, lb_nCC, ub_nCC, lb_UCC, ub_UCC):
    nCC_array = np.random.randint(lb_nCC, ub_nCC + 1, size=(popsize, 1))
    UCC_array = lb_UCC + np.random.rand(popsize, 1) * (ub_UCC - lb_UCC)
    fitness_array = np.zeros((popsize, 1))

    for i in range(popsize):
        PCC = hitung_PCC(nCC_array[i], UCC_array[i], 21)
        fitness_array[i] = hitung_fitness(PCC)

    return np.hstack((nCC_array, UCC_array, fitness_array))
